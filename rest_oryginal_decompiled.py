...
import os, bottle, json, logging, hashlib, traceback, time, itertools
...
from pim.services.accountids import LOCAL_BBM_ACCOUNTID, UNIFIED_CONTACTS_ACCOUNTID, LOCAL_CONTACTS_ACCOUNTID, LOCAL_SIMCONTACTS_ACCOUNTID
import string
...
class RestService(object):
    ...
    def __init__(self):
        ...
        def calendars_update(client_session, account_id, id):
            ...
            data = self.json_request()
            filtered_data = {k: v for k, v in  if k in editable_calendar_properties}
        ...
        def filtered_calendar_settings():
            ...
            fp = {k: v for k, v in  if is_calendar_settings_key(k)}
    ...
    def _save_or_send(self, client_session, account_id, action, orig_msg_id, ics_attachment=None, to_list_override=None, from_addr=None, reply_to_addr=None, provider_data=None):
        ...
        retval = message.send_message(client_session, account_id=account_id, to=to_list, body=body, body_plain_text=body_plain_text, from_addr=from_addr, body_type=body_content_type, subject=subject, cc=cc_list, bcc=bcc_list, options=options, attachments=attachment_list, action=action, orig_msg_id=orig_msg_id, draft_id=draft_id, references_sync_id=references_sync_id, reply_to_addr=reply_to_addr, provider_data=provider_data, secure_send_id=secure_send_id)
    ...
    def start(self):
        ...
        try:
            ...
            logger.exception("Failed to load route cache; please create it.")
            return

        if routes != self.rest.router.routes:
            logger.error("Route cache is out of date; please update it.")
            return
...
# okay decompiling rest_oryginal.pyc
